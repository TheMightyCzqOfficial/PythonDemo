...
def second():
    ...
    proxies = {
        ...
        'http': '110.243.10.222:9999'
    }
    n=2
    for i in range(20):
        url0 = "https://dymnt.com/zt/huaxiang/index_"+str(n)+".html"
        n=n+1
        ...
        for i in total[8:]:
            ...
            for i in range(num):
                ...
                res = worker_session.get(url, proxies=proxies, verify=False, stream=True, headers=header, timeout=500)
                ...
                urls3 = re.findall('<img alt="" src="(.*?)" />',content1)
                # print(str(urls1))
                # print(str(urls))
                # print(str(urls2))
                if (str(urls) == "[]" and str(urls1) == "[]" and str(urls3)=="[]"):
                    aa = urls2[0]
                    bb = aa.replace("https://img.dymnt.com/", "")
                    cc = "https://img.dymnt.com/" + bb
                    r = requests.get(cc, headers=header)
                if (str(urls) == "[]" and str(urls2) == "[]"and str(urls3)=="[]"):
                    aa = urls1[0]
                    bb = aa.replace("https://img.dymnt.com/", "")
                    cc = "https://img.dymnt.com/" + bb
                    r = requests.get(cc, headers=header)
                if (str(urls1) == "[]" and str(urls2) == "[]"and str(urls3)=="[]"):
                    aa = urls[0]
                    bb = aa.replace("https://img.dymnt.com/", "")
                    cc = "https://img.dymnt.com/" + bb
                    r = requests.get(cc, headers=header)
                if (str(urls) == "[]" and str(urls1) == "[]" and str(urls2)=="[]"):
                    aa = urls[3]
                    bb = aa.replace("https://img.dymnt.com/", "")
                    cc = "https://img.dymnt.com/" + bb
                    r = requests.get(cc, headers=header)
                ...
                name = "D:\\huaxiang\\" + biaoti + "\\" + biaoti + "  " + str(a) + ".jpg"
